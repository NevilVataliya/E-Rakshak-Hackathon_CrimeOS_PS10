"""
Crime OS AI — Persistent RAG Synthetic Benchmark Generator v3

Key improvements:
  1. Generates benchmark from the ACTUAL PRODUCTION collection (police_sops_v3)
  2. Adds target_text_fingerprint field for fuzzy matching during evaluation
  3. Expands allowed_page_window to ±2 pages
  4. Scrolls more points for better stratification
  5. Deterministic test case IDs for reproducibility
"""
...
from config import QDRANT_HOST, QDRANT_PORT, get_agent_llm, COLLECTION_NAME
...
def generate_benchmark_dataset():
    ...
    print(f"[+] Initializing Persistent RAG Synthetic Benchmark Generator v3 for '{collection_name}'...")
    ...
    # Scroll ALL points from Qdrant (paginate if needed)
    all_points = []
    offset = None
    while True:
        ...
        if next_offset is None or len(scroll_res) == 0:
            break
        offset = next_offset

    print(f"[+] Scrolled {len(all_points)} total points from Qdrant collection '{collection_name}'.")
    ...
    for pt in all_points:
        ...
        if spec in specialist_chunks:
            ...
            chunk_text = payload.get("text", payload.get("clean_text", ""))

            # Extract a 50-80 char fingerprint from the middle of the chunk for fuzzy matching
            clean_text = payload.get("clean_text", chunk_text)
            mid_point = len(clean_text) // 3
            fingerprint = clean_text[mid_point:mid_point + 80].strip() if len(clean_text) > 100 else clean_text[:80]
    ...
    for spec, chunks in specialist_chunks.items():
        ...
        for item in sampled_subset:
            ...
            # Expanded page window: ±2 pages
            window = list(range(max(1, page_num - 2), page_num + 3))
...
SECTION PATH: {item.get('section_path', 'General')}
TEXT SNIPPET:
{item['text_snippet']}
...
"""
            try:
                ...
                test_case = {
                    ...
                    "ground_truth_binding": {
                        ...
                        "target_text_fingerprint": item["text_fingerprint"]
                    },
    ...
    for i in range(max_comp):
        ...
        targets = []
        for chunk_item in [c_chunk, b_chunk, s_chunk, v_chunk]:
            pn = chunk_item["page_number"]
            targets.append({
                "target_specialist": chunk_item["specialist"],
                "source_document": chunk_item["source_document"],
                "page_number": pn,
                "allowed_page_window": list(range(max(1, pn - 2), pn + 3)),
                "point_id": chunk_item["point_id"],
                "target_text_fingerprint": chunk_item["text_fingerprint"]
            })
...
"""
    ...
    output_path = os.path.join(output_dir, "rag_benchmark_v3.json")
    ...
    print(f"\n[+] SUCCESS! Persistent RAG Benchmark v3 dataset saved to '{output_path}'.")
    print(f"    - Total Test Cases Generated: {len(test_cases)}")
    print(f"    - Aligned to Production Collection: {collection_name}")
