...
sys.path.append('/app')
import json
from app.rag.qdrant_client import get_query_embedding, get_qdrant_client, COLLECTION_NAME, compute_bm25_score, tokenize_text
from app.rag.query_optimizer import enrich_query_for_universal_rag, extract_universal_legal_terms
...
extracted_triggers = extract_universal_legal_terms(query)
boosted_triggers = " ".join(extracted_triggers * 5)
sparse_q = f"{boosted_triggers} {query}"

print(f"Extracted Triggers: {extracted_triggers}")
print(f"Sparse Q: {sparse_q[:200]}...")

query_vector = get_query_embedding(dense_q)
query_filter = Filter(
    should=[
        FieldCondition(key="target_specialist", match=MatchValue(value="cyber_financial_intel_specialist")),
...
query_tokens = tokenize_text(sparse_q)
sparse_scored = []
