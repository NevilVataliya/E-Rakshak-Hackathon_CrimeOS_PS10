...
export default function AgentFlowGraph({ activeStep = 4, status = 'APPROVED' }: AgentFlowGraphProps) {
  const steps = [
    ...
    <div className="glass-panel rounded-2xl p-5 mb-6">
      ...
      <div className="flex items-center justify-between border-b border-slate-800 pb-3 mb-4">
        ...
        <span className={`inline-flex items-center gap-1 rounded-md px-2.5 py-0.5 text-[10px] font-extrabold uppercase ${
          status === 'APPROVED'
            ? 'border border-emerald-500/40 bg-emerald-500/10 text-emerald-300 shadow-md shadow-emerald-500/20'
          ...
          return (
            <div
              ...
              className={`relative rounded-xl p-3 text-center transition-all ${
                isCurrent
                  ? 'border border-cyan-500/60 bg-cyan-500/20 text-white shadow-lg shadow-cyan-500/30'
                  : isCompleted
