...
export default function CommandPaletteDialog({ open, onOpenChange }: CommandPaletteDialogProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 dark:bg-black/80 backdrop-blur-md">
      
      <div className="w-full max-w-xl rounded-xl border border-slate-300 dark:border-white/10 bg-white dark:bg-[#0d1322] shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-150">
        ...
        <Command className="w-full font-sans text-xs">
          ...
          <div className="flex items-center border-b border-slate-200 dark:border-white/10 px-3">
            ...
            <Command.Input
              ...
              className="h-11 w-full bg-transparent text-xs text-slate-900 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:outline-none"
            />
          </div>
          ...
          <Command.List className="max-h-80 overflow-y-auto p-2 space-y-1">
            ...
            <Command.Group heading="PIPELINE MODULES" className="px-2 py-1 text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">
              <Command.Item
            ...
            <Command.Group heading="STATUTORY PROVISIONS" className="px-2 py-1 text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">
              <Command.Item
                onSelect={() => handleSelect('/investigation')}
                className="flex cursor-pointer items-center gap-2 rounded px-2 py-1.5 text-xs text-slate-800 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800"
              >
          ...
          <div className="flex items-center justify-between border-t border-slate-200 dark:border-white/10 px-3 py-1.5 text-[10px] text-slate-500 dark:text-slate-400">
            <span>Use ↑↓ to navigate, Enter to select</span>
            <button onClick={() => onOpenChange(false)} className="hover:text-slate-700 dark:hover:text-slate-300">ESC to close</button>
          </div>
